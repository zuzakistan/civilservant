	...
	request( IND_NEWS_URL, function ( err, res, body ) {
		if ( !err ) {
			...
			try {
				...
				if ( e instanceof SyntaxError ) {
					bot.shout( bot.config.irc.control, 'independent feed playing up' );
					return;
				} else {
					bot.shout( bot.config.irc.control, 'indy feed really playing up' );
					// throw e;
				}
			}
	...
	request( I100_NEWS_URL, function ( err, res, body ) {
		if ( !err ) {
			...
			try {
				...
				if ( e instanceof SyntaxError ) {
					bot.shout( bot.config.irc.control, 'independent100 feed playing up' );
					return;
				} else {
					bot.shout( bot.config.irc.control, 'indy100 feed really playing up' );
					// throw e;
				}
			}
